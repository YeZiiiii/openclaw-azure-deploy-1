...
## Copilot Agent 接入 OpenClaw 方式对比 / Copilot Agent Connection Methods

| | 方案 A: Azure Bot + Teams | 方案 B: Copilot Studio + MCP |
|---|---|---|
| **部署速度** | 较快（需额外上传 Teams App Manifest） | 很快（部署即就绪，只需填 API Key） |
| **协议** | Bot Framework Webhook | MCP (Model Context Protocol) |
| **支持场景** | Teams 群聊、频道、个人聊天、Copilot Agent | Copilot Studio Agent |
| **认证** | Azure Bot JWT（自动） | API Key（`x-api-key` header） |
| **部署后额外步骤** | 创建 Teams App Manifest → 打包上传 | SSH 获取 MCP API Key → Copilot Studio 中填入 |

### 方案 A 步骤路线 / Method A Steps

> 1 → 2 → 3 → **Microsoft Teams 配置** → 上传 Teams App Manifest

需要在第一步创建Azure Bot Service并准备 **Teams Bot App ID**和**Teams Bot App Password**。



### 方案 B 步骤路线 / Method B Steps

> 1 → 2 → 3 → **Copilot Studio MCP 配置**

无需创建Azure Bot Service。
...
Azure OpenAI 支持两种认证方式（二选一）：

- **API Key 模式**（默认）：填写 Endpoint + Deployment Name + API Key，三个参数要么全填要么全空。
- **Managed Identity 模式（Entra ID）**：适用于 Azure OpenAI 已禁用密钥访问的场景。填写 Endpoint + Deployment Name + Resource ID，三个参数要么全填要么全空。部署者需要对 Azure OpenAI 资源有 Owner 或 User Access Administrator 权限。

**Resource ID 格式**：
```
/subscriptions/{订阅ID}/resourceGroups/{资源组名}/providers/Microsoft.CognitiveServices/accounts/{资源名}
```
获取方式：Azure Portal → Azure OpenAI 资源 → Properties → Resource ID
...
如果您希望部署完成后立即接入 Microsoft Teams，作为Teams bot，请先准备以下信息：
...
4. 填写表单中的参数：
  ...
  - `azureOpenAiAuthMode`：认证模式，选 `API Key` 或 `Managed Identity (Entra ID)`
  - `azureOpenAiApiKey`：API Key 模式下填写，MI 模式下留空
  - `azureOpenAiResourceId`：MI 模式下填写 Azure OpenAI 资源的完整 Resource ID，API Key 模式下留空
...
7. 部署完成后，点击左侧的**输出 (Outputs)**，记录以下重要信息：
   ...
   - `mcpEndpointUrl` (MCP 服务器端点 URL，用于 Copilot Studio 配置)
   - `mcpAuthHeaderName` (MCP 认证 Header 名，固定为 `x-api-key`)
   - `mcpApiKeyNote` (获取 MCP API Key 的命令提示)
...
az deployment group create \
  ...
  --template-uri https://raw.githubusercontent.com/YeZiiiii/openclaw-azure-deploy-1/main/azuredeploy.json \
  --parameters \
...
[package模板](copilot-agent-package\manifest.json)，需要替换<ReplacedwithYourAzureBotID>内容为第一步的Azure Bot App ID。
...
## Copilot Studio MCP 配置

部署完成后，模板已自动在 VM 上安装并启动了 MCP Bridge 服务。要在 Copilot Studio 中接入 OpenClaw：

1. 登录 VM 获取 MCP API Key：
   ```bash
...
6. **API Key value**：粘贴步骤 1 获取的 key
7. 保存后自动发现 `openclaw_chat` 和 `openclaw_status` 两个工具
...
Azure OpenAI supports two authentication modes (choose one):

- **API Key mode** (default): provide Endpoint + Deployment Name + API Key; all three must be provided or all left empty.
- **Managed Identity mode (Entra ID)**: for Azure OpenAI resources with key access disabled. Provide Endpoint + Deployment Name + Resource ID; all three must be provided or all left empty. The deploying user must have Owner or User Access Administrator role on the Azure OpenAI resource.

**Resource ID format**:
```
/subscriptions/{subscription-id}/resourceGroups/{resource-group}/providers/Microsoft.CognitiveServices/accounts/{resource-name}
```
Find it at: Azure Portal → Azure OpenAI resource → Properties → Resource ID
...
4. Fill in the deployment parameters:
   ...
   - `azureOpenAiAuthMode`: authentication mode, choose `API Key` or `Managed Identity (Entra ID)`
   - `azureOpenAiApiKey`: required for API Key mode, leave empty for MI mode
   - `azureOpenAiResourceId`: required for MI mode (full Azure OpenAI Resource ID), leave empty for API Key mode
...
7. After deployment finishes, open **Outputs** on the left and record the following:
   ...
   - `mcpEndpointUrl` (MCP server endpoint URL for Copilot Studio configuration)
   - `mcpAuthHeaderName` (MCP auth header name, always `x-api-key`)
   - `mcpApiKeyNote` (command to retrieve the MCP API Key)
...
After deployment, the MCP Bridge service is automatically installed and running on the VM. To connect OpenClaw in Copilot Studio:

1. SSH into the VM and retrieve the MCP API Key:
   ```bash
   sudo grep MCP_API_KEY /etc/openclaw/openclaw.env
   ```
2. Open Copilot Studio → your Agent → add **Tool** → select **MCP**
3. **Server URL**: enter the `mcpEndpointUrl` from deployment outputs (e.g., `https://<hostname>/mcp`)
4. **Authentication**: select **API Key**
5. **Header name**: `x-api-key`
6. **API Key value**: paste the key from step 1
7. Save, and it will auto-discover `openclaw_chat` and `openclaw_status` tools

Verify:
```bash
sudo systemctl status openclaw-mcp --no-pager
